# Documentation Index

This folder is organized so a new engineer can move from product understanding to implementation, deployment, and roadmap planning quickly.

## Product and Architecture

- [Project Architecture](project/architecture.md)
- [Roadmap and Backlog](project/roadmap.md)
- [ClickUp Update Package](project/clickup-update.md)
- [GitHub Desktop and Repository Guide](project/github-desktop-guide.md)

## Operations

- [Deployment Guide](operations/deployment.md)
- [GoDaddy Domain Setup](operations/domain-setup-godaddy.md)
- [Local PostgreSQL Setup](operations/postgres-local-setup.md)
- [Firebase Google Auth Setup](operations/firebase-auth-setup.md)

## Source Material Included in Repo

- [Gains Product Specification](product/gains-product-spec.md)
- [Stock Research Playbook](research/stock-research-playbook.md)

## Suggested Onboarding Order

1. Read the root [README](../README.md)
2. Read [Project Architecture](project/architecture.md)
3. Read [Deployment Guide](operations/deployment.md)
4. Review [Roadmap and Backlog](project/roadmap.md)
5. Use [ClickUp Update Package](project/clickup-update.md) for delivery tracking
